...
find_best_params = function(res){
  m = -res$pred_return*res$nb_pred
  inds = which(m == max(m),arr.ind = TRUE)
  sig = colnames(m)[inds[2]]
  c_ = rownames(m)[inds[1]]
...
tresh_list = c(0.01,0.05,0.1)
y_lagReturn[is.na(y_lagReturn)]=0

c_array = c((1:5)*4,20+(1:10)*5,100)
sigma_array = c(0.032,0.064,0.096,0.128,0.16)
...
tresh = 0.25
# first training ----------------------------------------------------------
y_t = (y_lagReturn[t_start:t]<=quantile(x = y_lagReturn[t_start:t], tresh))*1

res = full_cross_validation(
  X=X[t_start:t],
  y=y_t,
  y_lagReturn = y_lagReturn[t_start:t],
  par_vec_2 = sigma_array,
  tresh = tresh,
  doOutliers = FALSE,
  verbose = FALSE
)

opt_params = find_best_params(res)

#fit model
fit = ksvm(x=as.matrix(X[t_start:t]),y=factor(as.matrix(y_t)),
           kernel=rbfdot(sigma = as.numeric(opt_params$sigma)),C=as.numeric(opt_params$C))


# rolling training --------------------------------------------------------
pred_array = y_lagReturn*0
while(t<length(y)){
  t = t+1
  
  pred = kernlab::predict(object = fit,newdata=as.matrix(X[t,]))
  pred =  as.numeric(levels(pred))[pred]
  pred_array[t] = pred
  
  if(t>max_train_window){
    t_start = t - max_train_window
  }else{
    t_start = 0
  }
  t_start = 0
  ...
  if(y_lagReturn[t]<=quantile(x = y_lagReturn[t_start:t], tresh)){
    # new interresting points, hence we retrain
    y_t = (y_lagReturn[t_start:t]<=quantile(x = y_lagReturn[t_start:t], tresh))*1
  ...
  if(t %% 100 == 0){
    ...
    print(paste(round(100*t/length(y),2),"%"))
  }
}

d = y_lagReturn[which(pred_array==1)]
sum(d)
mean(d)
sum(pred_array)/length(pred_array)
mean(y_lagReturn)
t.test(as.vector(y_lagReturn),as.vector(d))





# save(list = ls(),file ="rollingCrossVal25")
# save(list = ls(),file ="rollingCrossVal10")
# save(list = ls(),file ="rollingCrossVal5")
# save(list = ls(),file ="rollingCrossVal1")
# save(list = ls(),file ="rollingCrossVal_rmt_25")
# save(list = ls(),file ="rollingCrossVal_absorbtion_25")
# save(list = ls(),file ="rollingCrossVal_stationary_25")


# load("rollingCrossVal25")
